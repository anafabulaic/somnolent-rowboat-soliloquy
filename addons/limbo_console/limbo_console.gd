...
func error(p_line: String) -> void:
	print_line("[color=%s]%s" % [_output_error_color.to_html(), p_line] + "[/color]")


## Prints a warning message to the console and the output.
func warn(p_line: String) -> void:
	print_line("[color=%s]%s" % [_output_warning_color.to_html(), p_line] + "[/color]")
...
func _build_gui() -> void:
	...
	container.anchor_bottom = _options.height_ratio
	...
	var filter_holder := UIShaderStack.new()
	filter_holder.set_anchors_preset(Control.PRESET_FULL_RECT)
	filter_holder.mouse_filter = Control.MOUSE_FILTER_IGNORE
	add_child(filter_holder)
	
	var filter := ColorRect.new()
	filter.mouse_filter = Control.MOUSE_FILTER_IGNORE
	filter.material = preload("res://materials/_shaders/VHS_material.tres")
	filter.set_anchors_preset(Control.PRESET_FULL_RECT)
	filter_holder.add_child(filter)
...
func _greet() -> void:
	...
	#message = message.format({
		...
		#else:
			#info("[b]" + message + "[/b]")
	#info("exec cl_config.cfg")
	#info("exec mp_config.cfg")
	#info("Connection to relay coordinator established.")
	#info("Found a server.")
	#info("Sent PING to server")
	#info("Received PONG ack from server in 0.00 ms")
	#info("Received personality values from server in 0.00 ms")
	#error("Failed to initialize \"pscreamer_vol\" p_val: Invalid key")
	#error("Failed to initialize \"pscreamer_rand_noise\" p_val: Invalid key")
	#error("Couldn't find persistence data. Generating a fallback using defaults.")
	#info("Connecting to server.")
	#info("")
	#
	#var random_relay := str(randi_range(1,128))
	#
	#info("RELAY" + random_relay)
	#info("Map: srs_expanse")
	#info("Players: " + str(randi_range(1,2)) + " / 8")
	#info("Build 0433")
	#info("Server Number " + random_relay)
