...
func _process(delta: float) -> void:
	...
	if Input.is_key_pressed(KEY_SHIFT) and Input.is_action_just_pressed("3d_cursor_set_location") and _focused.get_parent().name.contains("Node3DEditorViewport"):
	...
	if Input.is_key_pressed(KEY_SHIFT) and Input.is_action_just_pressed("3d_cursor_show_pie_menu") and _focused.get_parent().name.contains("Node3DEditorViewport"):
...
func _main_window_focus_changed(control: Control):
	#print(control.name, " | ", control.get_parent())
	_focused = control
